...
if(isset($_POST['submit'])){ 
  if ((!@$_GET['program']) AND (!@$_GET['kegiatan'])) {
    echo '<div id="tampil_modal_save"><div id="modal">
          <div id="modal_atas"><i class="fa fa-info-circle fa-lg"></i> Error</div>
            <p>Pilih program dan kegiatan !</p>
          <a href="input_data.php"><button id="okesave">Oke</button></a>
        </div></div>';
  } else {
    if ((empty($_POST['rows'])) or (empty($_POST['rows_2'])) or (empty($_POST['rows_3']))) {
      echo '<div id="tampil_modal_save"><div id="modal">
          <div id="modal_atas"><i class="fa fa-info-circle fa-lg"></i> Error</div>
      ...
      foreach ($_POST['rows'] as $key => $count){
        ...
        if (!((!$_POST['sasaran_'.$count]) or (!$_POST['target_'.$count]))) {
          ...
          while ($cari_rows = mysqli_fetch_array($cari_target)) {
            $cari_programkegiatan = mysqli_query($koneksi,"select * from tb_programkegiatan where no_program='".$_GET['program']."' and id_kegiatan='".$_GET['kegiatan']."'");
            while ($program_rows = mysqli_fetch_array($cari_programkegiatan)) {
              $query_2 = "INSERT INTO `tb_laporan_programkegiatan`(`id_programkegiatan`, `no_target`, `no_tahun`, `nomor_skpd`) VALUES ('".$program_rows[0]."','".$cari_rows[0]."','$notahun','$nouser')";
      ...
      foreach ($_POST['rows_2'] as $key => $hitung ){
        ...
        while ($program_rows = mysqli_fetch_array($cari_programkegiatan)) {
          ...
          LEFT JOIN `tb_belanja` ON `tb_laporan_belanja`.`id_belanja` = `tb_belanja`.`id_belanja` WHERE `tb_laporan_belanja`.`id_ket`='1' AND `tb_laporan_belanja`.`nomor_skpd`='$nouser' AND `tb_laporan_belanja`.`id_programkegiatan`='".$program_rows[0]."' AND `tb_belanja`.`kode_belanja`='".$norek."'");
          if (mysqli_num_rows($validasi_bl)==0) {
            ...
            while ($data = mysqli_fetch_array($cari_belanja)) {
              $query_1 = "INSERT INTO `tb_laporan_belanja`(`id_laporan`, `id_belanja`, `id_ket`, `id_programkegiatan`, `nomor_skpd`, `no_tahun`) VALUES ('','".$data[0]."','1','".$program_rows[0]."','$nouser','$notahun')";
            ...
            <a href="input_data.php?program='.$_GET['program'].'&kegiatan='.$_GET['kegiatan'].'"><button id="okesave">Oke</button></a>
            </div></div>';
          }
        }
      }
      ...
      foreach ($_POST['rows_3'] as $key => $total ){
        ...
        while ($program_rows = mysqli_fetch_array($cari_programkegiatan)) {
          ...
          LEFT JOIN `tb_belanja` ON `tb_laporan_belanja`.`id_belanja` = `tb_belanja`.`id_belanja` WHERE `tb_laporan_belanja`.`id_ket`='2' AND `tb_laporan_belanja`.`nomor_skpd`='$nouser' AND `tb_laporan_belanja`.`id_programkegiatan`='".$program_rows[0]."' AND `tb_belanja`.`kode_belanja`='".$t_norek."'");
          if (mysqli_num_rows($validasi_btl)==0) {
            ...
            $cari_belanja = mysqli_query($koneksi,"SELECT * FROM `tb_belanja` ORDER BY `tb_belanja`.`id_belanja` DESC LIMIT 0,1");
            while ($data = mysqli_fetch_array($cari_belanja)) {
              $query_2 = "INSERT INTO `tb_laporan_belanja`(`id_laporan`, `id_belanja`, `id_ket`, `id_programkegiatan`, `nomor_skpd`, `no_tahun`) VALUES ('','".$data[0]."','2','".$program_rows[0]."','$nouser','$notahun')";
              mysqli_query($koneksi,$query_2) or die(mysql_error());
            }
          } else {
            echo '<div id="tampil_modal_save"><div id="modal">
...
?>
  ...
  <section>
    <div class="content">
    <div class="judul"><i class="fa fa-pencil fa-lg"></i> Input Data <?php echo $lastlogin ?></div>
    <div class="isi">
      <?php
        echo '<form method="GET">';
        /* Koding Menampilkan Program */
          echo '<p>Nama Program:';
            echo '<select name="program" class="input_data" onchange="this.form.submit()">';
              echo '<option name="combo" value="null">-- Pilih Program --</option>';
                ...
                if ($_GET['program']==$rows['0']) {
                    echo "<option name=\"combo\" value=\"".$rows['0']."\" selected>".$nouser.'.'.$rows['0'].' '.$rows['1']."</option>";
                  } else {  
                    echo "<option name=\"combo\" value=\"".$rows['0']."\">".$nouser.'.'.$rows['0'].' '.$rows['1']."</option>";
                  }
                }
          echo '</select></p>';
          /* Koding Menampilkan Kegiatan */
          echo '<p>Nama Kegiatan:';
            echo '<select name="kegiatan" class="input_data" onchange="this.form.submit()">';
              echo '<option name="combo" value="null">-- Pilih Kegiatan --</option>';
                $kegiatan = mysqli_query($koneksi,"SELECT * FROM tb_programkegiatan
                  LEFT JOIN `tb_kegiatan` ON `tb_programkegiatan`.`id_kegiatan` = `tb_kegiatan`.`id_kegiatan`  where no_program='".$_GET['program'].
                  "'");
                while ($rows = mysqli_fetch_array($kegiatan)) {
                if ($_GET['kegiatan']==$rows['0']) {
                    echo "<option name=\"combo\" value=\"".$rows['0']."\" selected>".$nouser.'.'.$_GET['program'].'.'.$rows['4'].' '.$rows['5']."</option>";
                  } else {  
                    echo "<option name=\"combo\" value=\"".$rows['0']."\">".$nouser.'.'.$_GET['program'].'.'.$rows['4'].' '.$rows['5']."</option>";
                  }
                }
          echo '</select></p>';
        echo '</form>';
        ...
        echo '</table>';
        if ((@$_GET['program']) and (!(@$_GET['kegiatan']=='null'))) {
        ...
        echo '<br /><button class="save" type="submit" name="submit" style="float: left"><i class="fa fa-save fa-lg"></i> Simpan</button> <a href="input_data.php" class="button_batal" style="margin-top: 0; padding-bottom: 12px; margin-left: 2px; float: left;"><i class="fa fa-plus fa-lg" style="  -webkit-transform: rotate(45deg);
      ...
      $(document).ready(function() {
            ...
            $("#add_btn").click(function(){
                ...
                $('#container').append(
                         ...
                         //+ '<td><input class="masuk" onkeyup="validAngka(this)" autocomplete="off" placeholder="Dana Kegiatan" id="dana_' + count + '" name="dana_' + count + '" type="text"></td>'
                         + '<td><a class="remove_item" onclick="return confirm(\'Hapus data ini?\')" href="#" style="color: #ea6153"><i class="fa fa-minus-square fa-lg"></i> Hapus</a>'
            ...
            $("#tambah_btn").click(function(){
                ...
                $('#container_2').append(
                         ...
                         + '<td><a class="remove_item_2" onclick="return confirm(\'Hapus data ini?\')" href="#" style="color: #ea6153"><i class="fa fa-minus-square fa-lg"></i> Hapus</a>'
            ...
            $("#tambah_button").click(function(){
                ...
                $('#container_3').append(
                         ...
                         + '<td><a class="remove_item_3" onclick="return confirm(\'Hapus data ini?\')" href="#" style="color: #ea6153"><i class="fa fa-minus-square fa-lg"></i> Hapus</a>'
